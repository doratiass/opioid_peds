...
model_3_shap <- read_csv("JAMIA/shap_rank.csv", show_col_types = FALSE) %>%
  select(-shap) %>%
  mutate(
    rank = case_when(
      #  is.na(rank) ~ 4,
...
ggsave(filename = file.path("fig4.pdf"), plot = ggplot2::last_plot(), 
       width = 30, height = 25, dpi = 300, units = "cm", bg = "white")
